...
class JSONMemoryStore:
    ...
    def __init__(self, base_dir: Optional[str] = None):
        ...
        print(f"JSON记忆存储初始化完成，基础目录: {self.base_dir}")
    ...
    def _get_available_therapy_types(self) -> List[str]:
        ...
        try:
            ...
            if os.path.exists(config_path):
                ...
                print(f"从配置文件加载了 {len(therapy_types)} 个治疗流派")

        except Exception as e:
            print(f"加载治疗流派配置失败: {str(e)}，将使用默认流派")
    ...
    def _init_files(self):
        ...
        for file_path in self.memory_files.values():
            if not file_path.exists():
                ...
                print(f"创建初始JSON文件: {file_path}")
            else:
                print(f"已存在JSON文件: {file_path}")
    ...
    def get_all_documents(self, collection_name: str) -> List[Dict[str, Any]]:
        ...
        if collection_name not in self.memory_files and collection_name.startswith("therapist_"):
            ...
            if not self.memory_files[collection_name].exists():
                with open(self.memory_files[collection_name], 'w', encoding='utf-8') as f:
                    json.dump([], f, ensure_ascii=False, indent=2)
                print(f"创建新的治疗流派JSON文件: {self.memory_files[collection_name]}")

        if collection_name not in self.memory_files:
            print(f"警告: 未找到集合 {collection_name} 的JSON文件")
            return []
        ...
        try:
            ...
            print(f"读取JSON文件 {self.memory_files[collection_name]} 时出错: {str(e)}")
            return []
        ...
        try:
            ...
            for i, doc in enumerate(documents):
                if doc.get("id") == document.get("id"):
                    print(f"文档ID {document.get('id')} 已存在，将更新而非添加")
            ...
            print(f"已添加/更新文档到 {collection_name}, ID: {document.get('id')}")
            return True

        except Exception as e:
            print(f"添加文档到 {collection_name} 时出错: {str(e)}")
            return False
    ...
    def update_document(self, collection_name: str, doc_id: str, document: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            print(f"已更新文档 {doc_id} 在 {collection_name}")
            return True

        except Exception as e:
            print(f"更新文档 {doc_id} 时出错: {str(e)}")
            return False
    ...
    def remove_document(self, collection_name: str, doc_id: str) -> bool:
        ...
        try:
            ...
            print(f"已从 {collection_name} 中删除文档 {doc_id}")
            return True

        except Exception as e:
            print(f"删除文档 {doc_id} 时出错: {str(e)}")
            return False
...
if __name__ == "__main__":
    ...
    print(f"读取到 {len(skills)} 条侧写师技能记忆")