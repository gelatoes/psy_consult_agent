...
from app.business_logic.agents import ProfilerAgent, SupervisorAgent, TherapistAgent
...
class ConsultationController(ABC):
    def __init__(
        self,
        ...
        interrupt_nodes = [
            #"initial_scale",
            "greet_user",
            "update_after_profiler_speak", 
            "initialize_cbt_topics",
            "update_after_stage_1",
            "update_after_stage_2",
            "update_after_stage_3",
            "update_after_stage_4",
            "final_scale"
        ]
        self.compiled_graph = self.graph.compile(interrupt_before=interrupt_nodes, checkpointer=self.checkpointer)
        super().__init__()
    ...
    def _load_cbt_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            config_path = os.path.join(base_dir, "data", "cbt_config.json")
    ...
    def _register_nodes(self):
        ...
        self.graph.add_node("update_after_profiler_speak", self._update_memory_after_profiler_speak)
        ...
        self.graph.add_node("update_after_stage_1", self._update_memory_after_stage_1)
        ...
        self.graph.add_node("update_after_stage_2", self._update_memory_after_stage_2)
        ...
        self.graph.add_node("update_after_stage_3", self._update_memory_after_stage_3)
        ...
        self.graph.add_node("update_after_stage_4", self._update_memory_after_stage_4)
    ...
    def _define_edges(self):
        ...
        # self.graph.add_edge("initial_scale", "greet_user")
        self.graph.add_edge("initialize", "greet_user")

        self.graph.add_edge("greet_user", "profiler_dialogue")
        self.graph.add_edge("profiler_dialogue", "update_after_profiler_speak")
        self.graph.add_edge("update_after_profiler_speak", "check_profiler_complete")
        self.graph.add_conditional_edges(
            "check_profiler_complete",
            self._should_continue_profiler,
            {True: "profiler_dialogue", False: "create_portrait"},
        )
        ...
        self.graph.add_edge("cbt_stage_1", "update_after_stage_1")
        self.graph.add_edge("update_after_stage_1", "check_stage_1_complete", )
        self.graph.add_conditional_edges(
        ...
        # CBT阶段2循环
        self.graph.add_edge("cbt_stage_2", "update_after_stage_2")
        self.graph.add_edge("update_after_stage_2", "check_stage_2_complete")
        self.graph.add_conditional_edges(
        ...
        # CBT阶段3循环
        self.graph.add_edge("cbt_stage_3", "update_after_stage_3")
        self.graph.add_edge("update_after_stage_3", "check_stage_3_complete")
        self.graph.add_conditional_edges(
        ...
        # CBT阶段4循环
        self.graph.add_edge("cbt_stage_4", "update_after_stage_4")
        self.graph.add_edge("update_after_stage_4", "check_stage_4_complete")
        self.graph.add_conditional_edges(
    ...
    async def start_new_session(self, session_id: int) -> Dict[str, Any]:
        config = {"configurable": {"thread_id": session_id}}
        return await self.compiled_graph.ainvoke({}, config)

    async def process_user_survey(self, session_id: int, session_state: dict, user_message: str) -> Dict[str, Any]:
        if session_state["current_phase"] == "survey":
            session_state['initial_scales_result']=user_message
        elif session_state["current_phase"] == "final_survey":
            session_state["scales_result_after_consultation"]=user_message
        session_state.pop("wait_for_user_input", None)
        config = {"configurable": {"thread_id": session_id}}
        self.compiled_graph.update_state(config, session_state)
        return await self.compiled_graph.ainvoke(None, config)
    
    async def process_user_message(self, session_id: int, session_state: dict, user_message: str) -> Dict[str, Any]:
        session_state['dialogue_history'].append([user_message, "user"])
        session_state.pop("wait_for_user_input", None)
        config = {"configurable": {"thread_id": session_id}}
        self.compiled_graph.update_state(config, session_state)
        return await self.compiled_graph.ainvoke(None, config)
    ...
    async def _initialize_state(self, state: Dict[str, Any]) -> Dict[str, Any]:
        state = {
            "current_student_basic_info": {},           # 学生信息
            "shared_memory": {},                        # 侧写师, 咨询师记忆
            "supervisor_working_memory": {},            # 指导员记忆
            "dialogue_history": [],                     # 历史对话
            "psychological_portraits": {},              # 心理画像
            "current_phase": 'initial',                 # 当前阶段
            "initial_scales_result": {},                # 初次量表
            "scales_result_after_consultation": {},     # 最终量表
            "is_profile_complete": False,               # 侧写阶段完成标志
            "is_consultation_complete": False,          # 咨询阶段完成标志
            "current_profile_dialogue_index": 0,        # 侧写对话轮数
            "current_consultation_dialogue_index": 0,   # 咨询对话轮数
            "is_complete": False,
            "selected_therapist_type": self.selected_therapist.therapy_type,
            "evaluation_result": {}
        }
        ...
        # greeting = "您好！欢迎使用心理咨询系统。我是您的心理侧写师，接下来我会和您聊一聊，了解您的情况。在开始对话前，请您先完成一份初始问卷。"
        greeting = "您好！欢迎使用心理咨询系统。我是您的心理侧写师，接下来我会和您聊一聊，了解您的情况。您最近有什么烦心事吗？"
        state["dialogue_history"].append([greeting, "侧写师"])
        state["current_phase"] = "profiling"
        state["wait_for_user_input"] = True
        return state

    async def _handle_initial_scale(self, state: Dict[str, Any]) -> Dict[str, Any]:
        output = "感谢您的问卷,请问您最近有什么想和我分享的吗？"
        state["dialogue_history"].append([output, "侧写师"])
        state["current_phase"] = "profiling"
        state["wait_for_user_input"] = True
        return state
    ...
    async def _handle_profiler_dialogue(self, state: Dict[str, Any]) -> Dict[str, Any]:
        """处理侧写师对话阶段(单轮)"""
        await self.profiler_agent.speak(state, self.supervisor_agent, self.memory_manager)
        state["wait_for_user_input"] = True 
        logger.info("侧写师说话")
        return state
    
    async def _update_memory_after_profiler_speak(self, state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _handle_therapist_selection(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        state["dialogue_history"].append([introduction, "咨询师"])
        state["wait_for_user_input"] = True
        return state
    ...
    async def _update_memory_after_stage_1(self, state: Dict[str, Any])-> Dict[str, Any]:
        return await self._update_memory_after_therapist_speak(state)
    
    async def _update_memory_after_stage_2(self, state: Dict[str, Any])-> Dict[str, Any]:
        return await self._update_memory_after_therapist_speak(state)
    
    async def _update_memory_after_stage_3(self, state: Dict[str, Any])-> Dict[str, Any]:
        return await self._update_memory_after_therapist_speak(state)
    
    async def _update_memory_after_stage_4(self, state: Dict[str, Any])-> Dict[str, Any]:
        return await self._update_memory_after_therapist_speak(state)
    
    async def _update_memory_after_therapist_speak(self, state: Dict[str, Any]) -> Dict[str, Any]:
        stage_name = state["current_cbt_stage"]
        await self._update_topic_scores(state)
        await self._evaluate_stage_completion(state)
    ...
    async def _cbt_therapist_speak(self, state: Dict[str, Any], stage_name: str, current_topic: str):
        ...
        if sub_questions:
            ...
            last_user = state["dialogue_history"][-1][0]
            recap = ""
            transition = ""
...
请直接返回一个完整的问题（包含举例）。"""
            ...
            state["dialogue_history"].append([composed, "咨询师"])
            # 递增子问题索引
        ...
        # 获取用户最近发言并提取关键词
        last_user = state["dialogue_history"][-1][0]        
        dialogue = "\n".join(f"{speaker}: {utterance}" for utterance, speaker in state["dialogue_history"][-4:])
        full_prompt = f"""
        ...
        state["dialogue_history"].append([response, "咨询师"])

    async def _update_topic_scores(self, state: Dict[str, Any]):
        """督导师评估用户回应并更新话题得分"""
        current_topic = state['current_topic']
        user_response = state["dialogue_history"][-1][0]
    ...
    async def _evaluate_stage_completion(self, state: Dict[str, Any]):
        """督导师评估阶段完成要素"""
        stage_name = state["current_cbt_stage"]
        ...
        dialogue = "\n".join(f"{speaker}: {utterance}" for utterance, speaker in state["dialogue_history"][-8:])
        # 构建评估提示词
        prompt = f"""
    ...
    async def _handle_evaluation(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        evaluation_result = await self.supervisor_agent.evaluate_therapist(state, therapy_type)
    ...
    async def _finalize_consultation(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        state["dialogue_history"].append([conclusion, "system"])
