# app/business_logic/services/prompt_loader.py
import os
import json
import yaml
...
class PromptLoader:
    ...
    def load_prompts(agent_type: str) -> Dict[str, str]:
        ...
        try:
            ...
            config_path = os.path.join(base_dir, "data", "prompts", f"{agent_type}_prompts.yaml")

            # 检查文件是否存在
            if not os.path.exists(config_path):
                print(f"找不到提示词配置文件: {config_path}")
                return {}
            ...
            print(f"成功加载了 {agent_type} 智能体的 {len(prompts)} 个提示词模板")
            return prompts
