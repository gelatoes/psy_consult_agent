...
class SupervisorAgent:
    ...
    async def assess_portrait(self, state: Dict[str, Any], true_portraits: Dict[str, Any]) -> str:
        ...
        logger.info("assess_portrait中的psychological_portraits")
    ...
    async def offer_cbt_stage_advice(self, state: Dict[str, Any], stage_name: str, stage_config: Dict[str, Any], current_topic: str) -> str:
        ...
        最近对话记录：
        {newline.join(state["dialogue_history"][-6:])}
    ...
    async def create_student_medical_record(self, state: Dict[str, Any], therapy_type: str,
                                            memory_manager: MemoryManager):
        """创建学生医疗记录"""
        try:
            # 准备医疗记录数据
            student_id = state["current_student_basic_info"]["id"]
            newline = "\n"  # 定义换行符变量以避免f-string中的反斜杠问题
            ...
            学生的基本信息:
            {json.dumps(state["current_student_basic_info"], ensure_ascii=False, indent=2)}
            ...
            整个对话记录：
            {newline.join(state["dialogue_history"])}
    ...
    async def get_risk_factors(self, state: Dict[str, Any]):
        """总结危险影响因素"""
        prompt_template = self.prompts.get("risk_factors")
        ...
        if result and isinstance(state.get("psychological_portraits"), dict):
            state["psychological_portraits"]["risk_factors"] = result
